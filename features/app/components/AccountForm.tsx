...
export function AccountForm({ t, onCancel, onSave, current }: AccountFormProps) {
  return (
    <form className="data-form" onSubmit={onSave}>
      ...
      <label>{t.accountName}<input name="name" required defaultValue={current?.name ?? ""} autoComplete="off" /></label>
      ...
      <label>{t.openingBalance}<input name="openingBalance" type="number" min="0" step="0.01" defaultValue={current?.opening_balance ?? 0} required /></label>
