...
export function CategoryForm({ t, locale, categories, current, userRole, onCancel, onSave }: CategoryFormProps) {
  const [kind, setKind] = useState<"income" | "expense">(current?.kind ?? "expense");
  const heading = current 
    ? (locale === "ne" ? "मुख्य श्रेणी सम्पादन गर्नुहोस्" : "Edit Main Category") 
    : (locale === "ne" ? "मुख्य श्रेणी थप्नुहोस्" : "Add Main Category");
  ...
  return <form className="data-form" onSubmit={onSave}>
    ...
    <label>{locale === "ne" ? "नेपाली नाम" : "Nepali Name"}<input name="name_ne" required defaultValue={current?.name_ne ?? ""} placeholder={locale === "ne" ? "जस्तै: घरायसी तथा दैनिक खर्च" : "e.g., घरायसी तथा दैनिक खर्च"} autoComplete="off" /></label>
    <label>{locale === "ne" ? "अंग्रेजी नाम" : "English Name"}<input name="name_en" required defaultValue={current?.name_en ?? ""} placeholder={locale === "ne" ? "जस्तै: Household & Daily Expenses" : "e.g., Household & Daily Expenses"} autoComplete="off" /></label>
