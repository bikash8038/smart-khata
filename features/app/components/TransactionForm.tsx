...
interface TransactionFormProps { t: Record<string, string>; locale: "en" | "ne"; accounts: Account[]; categories: Category[]; current: Transaction | null; initialKind: "income" | "expense"; onCancel: () => void; onSave: (event: FormEvent<HTMLFormElement>) => void; onSeedMainCategories: () => void }

export function TransactionForm({ t, locale, accounts, categories, current, initialKind, onCancel, onSave, onSeedMainCategories }: TransactionFormProps) {
  ...
  return <form className="data-form transaction-form" onSubmit={onSave}>
    ...
    <label>{locale === "ne" ? "मुख्य श्रेणी" : "Main Category"}<select name="mainCategory" value={mainCategoryId} onChange={(event) => changeMainCategory(event.target.value)} required><option value="">{locale === "ne" ? "मुख्य श्रेणी छान्नुहोस्" : "Choose Main Category"}</option>{mainCategories.map((category) => <option key={category.id} value={category.id}>{label(category)}</option>)}</select>{mainCategories.length === 0 && <button type="button" className="seed-categories-button" onClick={onSeedMainCategories}>{locale === "ne" ? "डिफल्ट मुख्य श्रेणीहरू बनाउनुहोस्" : "Create default main categories"}</button>}</label>
